...
const BUILD = '2026-08-23-i';      // shown in Me -> Data; bump on every upload
...
function renderStats(v) {
  ...
  facts.innerHTML = `<div class="section-head">The numbers</div>` + [
    ...
    ['🏁', 'Shows finished', db.shows.filter(x => !x.dropped && x.total_episodes > 0
        && x.watched_episodes.length >= x.total_episodes).length],
...
function openMonthRecap(key, id) {
  ...
  const body = openSheet(`<div class="recap">
    <div class="recap-top">
      ${recapCollage(shows)}
      <div class="recap-veil"></div>
      <div class="recap-head">
        <div class="recap-eyebrow">Wrap-up</div>
        <div class="recap-title">${esc(prettyMonth(key))}</div>
        <div class="recap-big">${total.toLocaleString()}</div>
        <div class="recap-sub">things watched · about ${Math.floor(mins / 60)} hours</div>
      </div>
    </div>
    <div class="sheet-body">
      ${recapSlide('The shape of it', `
        <div class="split">
          <div><b>${st.tv}</b><span>episodes</span></div>
          <div><b>${st.movie}</b><span>films</span></div>
          <div><b>${active}<i>/${inMonth}</i></b><span>days active</span></div>
        </div>`)}
      ${active ? recapSlide('Every day', monthDayStrip(key).replace(/<div class="section-head">.*?<\/div>/, '')) : ''}
      ${recapSlide(Object.keys(shows).length ? 'What you kept coming back to' : 'Your most-watched shows',
                   recapPodium(shows, 3, true))}
      ${active ? '' : `<div class="note">Only the totals were recorded for this month — the
        per-day detail, ratings and moods came later.</div>`}
      ${recapFacts([
      ...
      <button class="btn gold" id="recap-done" style="width:100%">Nice</button>
    </div></div>`);
...
function openYearRecap(year, id) {
  ...
  const body = openSheet(`<div class="recap">
    <div class="recap-top tall">
      ${recapCollage(shows)}
      <div class="recap-veil"></div>
      <div class="recap-head">
        <div class="recap-eyebrow">Year in review</div>
        <div class="recap-title big">${esc(year)}</div>
        <div class="recap-big">${(tv + mv).toLocaleString()}</div>
        <div class="recap-sub">things watched · ${dayCount} days of screen time</div>
      </div>
    </div>
    <div class="sheet-body">
      ${recapSlide('The shape of it', `
        <div class="split">
          <div><b>${tv.toLocaleString()}</b><span>episodes</span></div>
          <div><b>${mv.toLocaleString()}</b><span>films</span></div>
          <div><b>${Object.keys(days).length}</b><span>days active</span></div>
        </div>`)}
      ${yearMonthChart(year) ? recapSlide('Across the year',
          yearMonthChart(year).replace(/<div class="section-head">.*?<\/div>/, '')) : ''}
      ${recapSlide(Object.keys(shows).length ? 'Your top shows' : 'Your most-watched shows',
                   recapPodium(shows, 5, true))}
      ${yearMilestones(year) ? recapSlide('Milestones',
          yearMilestones(year).replace(/<div class="section-head">.*?<\/div>/, ''))
        : `<div class="note">No journal entries survive for ${esc(year)}, so milestones
           aren't available — the monthly totals above are still accurate.</div>`}
      ${recapFacts([
        avgR ? ['⭐', 'Average rating', `${avgR.toFixed(1)} · ${RATING_WORDS[Math.round(avgR)] || ''}`] : null,
      ...
      <div class="tier big"><span class="tier-ic">${tier[0]}</span>
        <b>${tier[1]}</b><span>${tier[2]}</span></div>
      <button class="btn gold" id="recap-done" style="width:100%">Claim it</button>
    </div></div>`);
  body.querySelector('#recap-done').onclick = async () => { await markRecapSeen(id); closeSheet(); render(); };
}
...
/* Back-gesture handling.
   ...
   The previous version called replaceState at boot, which REPLACED the only
   history entry rather than adding one — so back had nothing to pop and the
   browser left the app. Now we keep a spare entry permanently in front of a
   guard entry, and re-arm it every time it gets consumed:

     [guard] [buffer]   <- you are here
     back  -> lands on guard, we act, then push a fresh buffer

   Only when a back arrives with no sheet open and you are already on the first
   tab do we let it through and actually leave. */
function armBack() {
  try { history.pushState({ mytv: 'buffer' }, ''); } catch { /* ignore */ }
}

window.addEventListener('popstate', () => {
  if (!$('#sheet').hidden) {        // 1. a detail sheet is open -> close it
    closeSheet();
    armBack();
    return;
  }
  if (view !== 'next') {            // 2. deeper tab -> step back to the first
    view = 'next';
    render();
    armBack();
    return;
  }
  history.back();                   // 3. already at the root -> leave the app
});
...
function boot() {
  // Guard entry, then a spare buffer on top of it. replaceState alone was the
  // bug: it swapped the current entry instead of adding one, leaving nothing
  // for the back gesture to pop.
  try { history.replaceState({ mytv: 'guard' }, ''); } catch { /* ignore */ }
  armBack();
