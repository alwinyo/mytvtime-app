...
const invalidateFeeds = () => { feedCache = { sig: null, next: null, soon: null }; };
...
async function fb(path) {
  ...
  if (hit) return hit;
  const data = await fetchJson('https://api.football-data.org/v4/' + path,
                               { headers: { 'X-Auth-Token': cfg.football } }, 12000);
...
async function computeNext(onProgress) {
  ...
  feedCache.sig = sig; feedCache.next = rows;
  return rows;
}
...
async function computeSoon(onProgress) {
  ...
  feedCache.sig = sig; feedCache.soon = rows;
  return rows;
}
...
async function renderNext(v) {
  ...
  const all = await computeNext((d, t) => { if (progEl) progEl.textContent = `${d}/${t}`; });
  v.innerHTML = '';
...
async function renderSoon(v) {
  ...
  const progEl = v.querySelector('.prog');
  const all = await computeSoon((d, t) => { if (progEl) progEl.textContent = `${d}/${t}`; });
  v.innerHTML = '';
  ...
  for (const [code, name] of Object.entries(FOOTBALL_COMPETITIONS)) {
    ...
    if (!Object.keys(days).length) {
      body.appendChild(emptyState("Couldn't load fixtures",
        'Check your football key, or wait a minute — the free tier allows 10 requests per minute.'));
      continue;
    }
