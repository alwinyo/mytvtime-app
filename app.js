...
const BUILD = '2026-08-23-h';      // shown in Me -> Data; bump on every upload
...
/* Android's back gesture used to close the whole app while a sheet was open.
   Pushing a history entry when a sheet opens means back pops that first. */
...
function openSheet(html) {
  ...
  if (!sheetPushed) {
    sheetPushed = true;
    try { history.pushState({ sheet: true }, ''); } catch { /* ignore */ }
  }
  return $('#sheet-body');
}
...
window.addEventListener('popstate', () => {
  ...
  // No sheet open: step back through tabs rather than leaving the app, and
  // re-arm the stack so the gesture keeps working.
  if (view !== 'next') {
    view = 'next';
    render();
    try { history.pushState({ base: true }, ''); } catch { /* ignore */ }
  }
});
...
function boot() {
  // A known base entry, so the very first back gesture has something to pop
  // instead of closing the app.
  try { history.replaceState({ base: true }, ''); } catch { /* ignore */ }
...
document.querySelectorAll('#tabbar button').forEach(b => {
  b.onclick = () => {
    ...
    view = b.dataset.view;
    if (view !== 'next') { try { history.pushState({ tab: view }, ''); } catch { /* ignore */ } }
    render();
  };
});
