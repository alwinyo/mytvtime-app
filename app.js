...
const BUILD = '2026-08-23-k';      // shown in Me -> Data; bump on every upload
...
function renderData(v) {
  ...
  app.innerHTML += `<div class="kv"><span>History depth</span><b>${window.history.length}</b></div>`;
...
function noteBack(what) {
  backLog.unshift(`${stamp().slice(11)} ${what} (depth ${window.history.length})`);
