...
const BUILD = '2026-08-23-f';      // shown in Me -> Data; bump on every upload
...
function recapCollage(counts) {
  const posters = Object.keys(counts)
...
function openMonthRecap(key, id) {
  ...
  const body = openSheet(`<div class="recap">
    ...
    <div class="sheet-body">
      ...
      ${Object.keys(shows).length ? recapSlide('What you kept coming back to', recapPodium(shows, 3)) : ''}
      ${recapFacts([
...
function openYearRecap(year, id) {
  ...
  const body = openSheet(`<div class="recap">
    ...
    <div class="sheet-body">
      ...
      ${recapSlide('Across the year', yearMonthChart(year).replace(/<div class="section-head">.*?<\/div>/, ''))}
      ${Object.keys(shows).length ? recapSlide('Your top shows', recapPodium(shows, 5)) : ''}
      ${recapSlide('Milestones', yearMilestones(year).replace(/<div class="section-head">.*?<\/div>/, ''))}
      ${recapFacts([
        avgR ? ['⭐', 'Average rating', `${avgR.toFixed(1)} · ${RATING_WORDS[Math.round(avgR)] || ''}`] : null,
