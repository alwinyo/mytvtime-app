...
const BUILD = '2026-08-23-d';      // shown in Me -> Data; bump on every upload
...
const fbInflight = new Map();      // same path requested twice -> one request
let fbNextAllowed = 0;             // free tier is 10/min; keep ~7s between calls
...
async function fb(path) {
  ...
  // A failure used to cache nothing, so every re-render retried instantly and
  // kept us permanently rate limited. Remember failures for 2 minutes too.
  const bad = cacheGet('fbfail:' + path, 120000);
  if (bad) { fb.lastError = bad; return null; }

  if (fbInflight.has(path)) return fbInflight.get(path);

  const run = (async () => {
    const wait = fbNextAllowed - Date.now();
    if (wait > 0) await new Promise(r => setTimeout(r, wait));
    fbNextAllowed = Date.now() + 7000;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        fb.lastError =
          res.status === 403 ? `Key rejected (403). ${detail || 'Check the football key in setup.'}`
        : res.status === 429 ? `Rate limited (429). ${detail || 'Free tier allows 10 requests/minute.'} Waiting 2 min before retrying.`
        : `HTTP ${res.status}. ${detail}`.trim();
        cacheSet('fbfail:' + path, fb.lastError);
        return null;
      }
      ...
      fb.lastError = e.name === 'AbortError'
        ? 'Timed out after 12s.'
        : `Network or CORS failure: ${e.message}`;
      cacheSet('fbfail:' + path, fb.lastError);
      return null;
    } finally {
      fbInflight.delete(path);
    }
  })();

  fbInflight.set(path, run);
  return run;
}
fb.lastError = null;
...
function renderStats(v) {
  ...
  v.appendChild(statCards([
    ...
    [db.shows.filter(x => !x.dropped).length, '', 'Shows']]));
  v.appendChild(statCards([
    [`${avg(rated.filter(h => h.type === 'tv'))} / ${avg(rated.filter(h => h.type === 'movie'))}`, '', 'Avg TV / film'],
    [topKey(plats)?.[0] || '—', '', 'Top platform']]));
  v.appendChild(statCards([[topKey(feels)?.[0] || '—', '', 'Signature mood']]));
...
function openMonthRecap(key, id) {
  ...
  const body = openSheet(`<div class="sheet-body">
    <div class="recap-eyebrow">Monthly wrap-up</div><div class="recap-title">${esc(prettyMonth(key))}</div>
...
function renderData(v) {
  ...
  if (footballEnabled()) {
    ...
    const out = el('div', 'muted', 'Free tier allows 10 requests per minute. Failures are remembered for 2 minutes to avoid making it worse.');
    foot.appendChild(dataRow('Test connection',
      'Calls the API once and reports the exact response.',
      'Test', async (b) => {
        b.disabled = true; out.textContent = 'Testing…';
        try {
          const res = await fetch('https://api.football-data.org/v4/competitions/PL',
            { headers: { 'X-Auth-Token': cfg.football } });
          let body = '';
          try { body = JSON.stringify(await res.json()).slice(0, 160); } catch { body = '(no body)'; }
          out.innerHTML = res.ok
            ? `<b style="color:#8BC34A">OK ${res.status}</b> — the key works.`
            : `<b style="color:#ff8a80">HTTP ${res.status}</b> ${esc(body)}`;
        } catch (e) {
          out.innerHTML = `<b style="color:#ff8a80">Blocked</b> ${esc(e.message)}`;
        }
        b.disabled = false;
      }));
    foot.appendChild(dataRow('Clear football cache',
      'Forgets cached fixtures and any remembered failure, then retries.',
      'Clear', () => {
        Object.keys(localStorage).filter(k => k.startsWith('fb:') || k.startsWith('fbfail:'))
          .forEach(k => localStorage.removeItem(k));
        toast('Football cache cleared'); render();
      }));
