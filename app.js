...
const BUILD = '2026-08-23-c';      // shown in Me -> Data; bump on every upload
...
async function migrateLegacy() {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    await sb('watch_history?on_conflict=media_type,item_id,episode_code', {
...
async function addHistory(type, itemId, code, when) {
  bumpAnalytics(type, +1, when);
  if (historyTableMissing) return null;
...
async function patchHistory(entry, fields) {
  ...
  if (historyTableMissing || !entry.id) return;
...
const footballEnabled = () => !!(cfg && cfg.football);

async function fb(path) {
  const key = 'fb:' + path;
  const hit = cacheGet(key, FOOTBALL_TTL);
  if (hit) return hit;
  // Keep the real reason so the UI can show it. football-data.org does allow
  // browser CORS, so a failure here is usually a bad key or the 10/min limit.
  try {
    const ctrl = new AbortController();
    const timer = setTimeout(() => ctrl.abort(), 12000);
    const res = await fetch('https://api.football-data.org/v4/' + path,
      { headers: { 'X-Auth-Token': cfg.football }, signal: ctrl.signal });
    clearTimeout(timer);
    if (!res.ok) {
      fb.lastError = res.status === 403 ? 'Key rejected (403). Check FOOTBALL key.'
                   : res.status === 429 ? 'Rate limited (429). Free tier allows 10 requests/minute — wait a minute.'
    ...
    fb.lastError = e.name === 'AbortError'
      ? 'Request timed out after 12s.'
      : `Network/CORS failure: ${e.message}`;
    return null;
  }
}
fb.lastError = null;
...
async function currentSeasonYear(code) {
  const info = await fb(`competitions/${code}`);
  const start = String(info?.currentSeason?.startDate || '').slice(0, 4);
  return /^\d{4}$/.test(start) ? +start : calendarSeason();
}
async function competitionEmblem(code) {
  const info = await fb(`competitions/${code}`);
  return info?.emblem || `https://crests.football-data.org/${code}.png`;
}
...
async function footballMatchdays(code, season) {
  const data = await fb(`competitions/${code}/matches?season=${season}`);
  const out = {};
...
async function computeNext(onProgress) {
  ...
  if (!found.timedOut) { feedCache.sig = sig; feedCache.next = rows; persistFeedCache(); }
  return rows;
}
...
async function computeSoon(onProgress) {
  ...
  if (!partial) { feedCache.sig = sig; feedCache.soon = rows; persistFeedCache(); }
  return rows;
}
...
function posterEl({ title, poster, badge, progress, mediaHtml, onOpen }) {
...
function openRow(r) {
  if (r.src === 'football') openFootball(r.id, r.name);
  else if (r.movie) openMovie(r.id, r.name);
  else openEpisode(r.id, r.name, r.code);
}

async function renderNext(v) {
  // Paint the last good result immediately if the library hasn't changed.
  const cached = feedCache.sig === libSignature() ? feedCache.next : null;
  if (!cached) {
  ...
  h.innerHTML = `<img src="${imgUrl(hero.backdrop || hero.poster, 'w780')}" alt="">
    <span class="hero-inner"><span class="hero-eyebrow">Up next</span>
  ...
  watch.onclick = async () => {
    ...
    invalidateFeeds();
    toast(`${hero.code || hero.name} logged`);
    openRow(hero); render();
  };
...
async function renderSoon(v) {
  // Paint the last good result immediately if the library hasn't changed.
  const cached = feedCache.sig === libSignature() ? feedCache.soon : null;
  if (!cached) {
  ...
  for (const [code, name] of Object.entries(FOOTBALL_COMPETITIONS)) {
    ...
    if (!Object.keys(days).length) {
      body.appendChild(emptyState("Couldn't load fixtures",
        fb.lastError || 'No fixtures returned. The season may not be published yet.'));
      continue;
    }
...
function renderLibrary(v) {
  ...
  v.appendChild(gridOf(shown.map(x => libKind === 'tv'
    ? { title: x.name, poster: x.poster_path,
        progress: x.total_episodes ? x.watched_episodes.length / x.total_episodes : -1,
...
function ratingBars() {
  ...
  return el('div', 'rbars', counts.map((n, i) =>
    `<div class="rbar"><span>${'★'.repeat(i + 1)}</span>
...
async function renderMe(v) {
  v.innerHTML = '';
  v.appendChild(segmented(
    [['Stats', 'stats'], ['Health', 'health'], ['Journal', 'journal'], ['Recaps', 'recaps'], ['Data', 'data']],
    meTab, t => { meTab = t; renderMe(v); }, 3));
  const body = el('div');
  v.appendChild(body);
  if (meTab === 'stats') renderStats(body);
  else if (meTab === 'health') await renderHealth(body);
  else if (meTab === 'journal') renderJournal(body);
...
function renderStats(v) {
  ...
  const done = db.shows.filter(s => !s.dropped && s.total_episodes > 0 && s.watched_episodes.length >= s.total_episodes).length;
  const flairs = [];
  ...
  v.appendChild(el('div', 'section-head', 'Your flair'));
  v.appendChild(el('div', 'flairs', flairs.map(f => `<span class="chip">${esc(f)}</span>`).join('')));
  v.appendChild(statCards([
    [Math.floor(total / 43800), 'mo', 'Months'],
    [Math.floor((total % 43800) / 1440), 'd', 'Days'],
    [Math.floor((total % 1440) / 60), 'h', 'Hours']]));
  v.appendChild(statCards([
    [eps.toLocaleString(), '', 'Episodes'],
    [films.length.toLocaleString(), '', 'Films']]));
  v.appendChild(statCards([
    [`${avg(rated.filter(h => h.type === 'tv'))}/${avg(rated.filter(h => h.type === 'movie'))}`, '', 'Avg TV / film'],
    [topKey(plats)?.[0] || 'N/A', '', 'Top platform']]));
  v.appendChild(statCards([[topKey(feels)?.[0] || 'N/A', '', 'Signature mood']]));
...
async function renderHealth(v) {
  ...
  for (const s of db.shows) {
    ...
    let total;
    if (isFootballShow(s)) {
      // Only fixtures that have kicked off count as a backlog.
    ...
    if (rem > 0 && rem <= 3 && !isFootballShow(s)) almost.push({ id: s.id, name: s.name, rem });
  }
  ...
  for (const h of history) {
    ...
    if (dt >= c30) mins30 += h.type === 'tv' ? EPISODE_MINUTES : MOVIE_MINUTES;
    if (dt >= c7 && h.type === 'tv') eps7++;
  }
  const daysToClear = Math.round(backlogMins / (mins30 > 0 ? mins30 / 30 : 1));
  ...
  v.appendChild(el('div', 'section-head', 'Clearance'));
  v.appendChild(el('div', 'progress', `<span style="width:${Math.min(pct, 100)}%"></span>`));
  v.appendChild(el('div', 'muted', `Library ${pct}% complete`));
  v.appendChild(statCards([
    [humanizeDays(daysToClear), '', 'To clear backlog'],
    [eps7, 'eps', 'Last 7 days'],
    [humanizeDays(streak), '', 'Streak']]));

  if (almost.length) {
    v.appendChild(el('div', 'section-head', 'Almost finished'));
    almost.slice(0, 6).forEach(a => {
      const b = el('button', 'line');
      b.innerHTML = `<b>${esc(a.name)}</b> — ${a.rem} left`;
      b.onclick = () => openShow(a.id, a.name);
      v.appendChild(b);
    });
  }

  // Started, not finished, nothing logged in 90 days.
  const stagnant = [];
  for (const s of db.shows) {
    if (s.dropped || isFootballShow(s)) continue;   // never flag a show mid international break
    const w = s.watched_episodes.length;
    if (!w || w >= (s.total_episodes || 1)) continue;
    const last = history.filter(h => h.type === 'tv' && h.item === String(s.id))
                        .map(h => h.at).sort().pop();
    if (!last) continue;
    const age = (now() - new Date(last.replace(' ', 'T') + 'Z')) / 86400000;
    if (age > 90) stagnant.push({ ...s, age: Math.round(age) });
  }
  if (stagnant.length) {
    v.appendChild(el('div', 'section-head', `Stagnant · ${stagnant.length}`));
    v.appendChild(el('div', 'muted', 'Started but untouched for over three months.'));
    stagnant.slice(0, 6).forEach(x => {
      const b = el('button', 'line');
      b.innerHTML = `<b>${esc(x.name)}</b> — ${humanizeDays(x.age)} ago`;
...
function renderJournal(v) {
  ...
  let month = '';
  for (const h of history.slice(0, journalLimit)) {
    const m = h.at.slice(0, 7);
    if (m !== month) { month = m; v.appendChild(el('div', 'section-head', m)); }
    const isTv = h.type === 'tv';
    const obj = isTv ? getShow(h.item) : getMovie(h.item);
    const b = el('button', 'log');
    b.innerHTML = `<img loading="lazy" src="${imgUrl(obj?.poster_path, 'w185')}" alt=""
  ...
  v.appendChild(el('div', 'muted',
    `Showing ${Math.min(journalLimit, history.length).toLocaleString()} of ${history.length.toLocaleString()}`));
  if (history.length > journalLimit) {
    ...
    all.onclick = () => { journalLimit = history.length; render(); };
    v.appendChild(all);
  }
}
...
function renderRecaps(v) {
  ...
  if (pending.length) {
    v.appendChild(el('div', 'section-head', 'New'));
    pending.forEach(r => {
      const b = el('button', 'btn gold');
      b.style.cssText = 'width:100%;margin-bottom:8px';
      b.textContent = r.kind === 'month' ? `${r.key} wrap-up` : `${r.key} year in review`;
      b.onclick = () => r.kind === 'month' ? openMonthRecap(r.key, r.id) : openYearRecap(r.key, r.id);
      v.appendChild(b);
    });
  }
  const seen = (db.seen_recaps || []).slice().sort().reverse();
  if (!seen.length && !pending.length) {
    v.appendChild(emptyState('No recaps yet', 'Keep watching — monthly and yearly wraps unlock automatically.'));
    return;
  }
  if (seen.length) {
    v.appendChild(el('div', 'section-head', 'Archive'));
    seen.forEach(id => {
      const b = el('button', 'btn');
      b.style.cssText = 'width:100%;margin-bottom:8px';
      const key = id.replace(/^(monthly|yearly)-/, '');
      b.textContent = id.startsWith('monthly') ? `${key} wrap-up` : `${key} year in review`;
      b.onclick = () => id.startsWith('monthly') ? openMonthRecap(key, id) : openYearRecap(key, id);
      v.appendChild(b);
    });
  }
}
...
function recapPodium(counts, limit) {
  const cards = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, limit).map(([sid, n], i) => {
    const s = getShow(sid);
    if (!s) return '';
    return `<div class="podium"><span class="rank">${i + 1}</span>
      ...
      <span><b>${esc(s.name)}</b><i>${n} episodes</i></span></div>`;
  }).filter(Boolean).join('');
...
function openMonthRecap(key, id) {
  ...
  const facts = [];
  if (ratings.length) facts.push(['⭐', 'Average rating', `${(ratings.reduce((a, b) => a + b, 0) / ratings.length).toFixed(1)} / 5`]);
  if (topKey(plats)) facts.push(['📡', 'Top platform', topKey(plats)[0]]);
  if (topKey(feels)) facts.push(['🎭', 'Mood of the month', topKey(feels)[0]]);
  if (topKey(days)) facts.push(['🔥', 'Biggest day', `${topKey(days)[1]} on ${topKey(days)[0].slice(8)}`]);
  if (Object.keys(days).length) facts.push(['📅', 'Days watched', `${Object.keys(days).length}`]);

  const body = openSheet(`<div class="sheet-body">
    <div class="recap-eyebrow">Monthly wrap-up</div><div class="recap-title">${esc(key)}</div>
    ${recapHero((st.tv + st.movie).toLocaleString(), 'Titles watched', `about ${Math.floor(mins / 60)} hours`)}
    <div class="stat-row"><div class="stat"><b>${st.tv}</b><span>Episodes</span></div>
      <div class="stat"><b>${st.movie}</b><span>Films</span></div></div>
    ${recapFacts(facts)}
    ${Object.keys(shows).length ? '<div class="section-head">Most watched</div>' + recapPodium(shows, 3) : ''}
    <button class="btn gold" id="recap-done" style="width:100%">Nice</button></div>`);
  body.querySelector('#recap-done').onclick = async () => { await markRecapSeen(id); closeSheet(); render(); };
}
...
function openYearRecap(year, id) {
  ...
  const dayCount = Math.floor((tv * EPISODE_MINUTES + mv * MOVIE_MINUTES) / 1440);
  const months = {};
  for (const h of history) if (h.at.startsWith(year)) months[h.at.slice(0, 7)] = (months[h.at.slice(0, 7)] || 0) + 1;

  const facts = [];
  if (ratings.length) facts.push(['⭐', 'Average rating', `${(ratings.reduce((a, b) => a + b, 0) / ratings.length).toFixed(1)} / 5`]);
  if (topKey(plats)) facts.push(['📡', 'Top platform', topKey(plats)[0]]);
  if (topKey(feels)) facts.push(['🎭', 'Signature mood', topKey(feels)[0]]);
  if (topKey(months)) facts.push(['📈', 'Busiest month', `${topKey(months)[0].slice(5)} · ${topKey(months)[1]}`]);
  if (topKey(days)) facts.push(['🔥', 'Biggest day', `${topKey(days)[1]} on ${topKey(days)[0]}`]);
  ...
  const body = openSheet(`<div class="sheet-body">
    <div class="recap-eyebrow">Year in review</div><div class="recap-title big">${esc(year)}</div>
    ${recapHero((tv + mv).toLocaleString(), 'Titles watched', `${dayCount} days on screen`)}
    <div class="stat-row"><div class="stat"><b>${tv.toLocaleString()}</b><span>Episodes</span></div>
      <div class="stat"><b>${mv.toLocaleString()}</b><span>Films</span></div></div>
    ${recapFacts(facts)}
    ${Object.keys(shows).length ? '<div class="section-head">Your top shows</div>' + recapPodium(shows, 5) : ''}
    <div class="tier"><b>${tier[0]}</b><span>${tier[1]}</span></div>
    <button class="btn gold" id="recap-done" style="width:100%">Claim it</button></div>`);
  body.querySelector('#recap-done').onclick = async () => { await markRecapSeen(id); closeSheet(); render(); };
}

function renderData(v) {
  v.appendChild(el('div', 'section-head', 'Backup'));
  v.appendChild(el('div', 'muted', `Library, ${history.length.toLocaleString()} history entries, analytics.`));
  const dl = el('button', 'btn gold', 'Download backup');
  dl.style.cssText = 'width:100%;margin-top:8px';
  dl.onclick = () => {
  ...
  v.appendChild(el('div', 'section-head', 'Repair'));
  v.appendChild(el('div', 'muted', 'Recount monthly totals from history if the charts look doubled.'));
  const fix = el('button', 'btn', 'Recount totals');
  fix.style.cssText = 'width:100%;margin-top:8px';
  fix.onclick = async () => {
    const a = {};
    for (const h of history) {
      if (h.at.length < 7) continue;
      const b = (a[h.at.slice(0, 7)] ||= { tv: 0, movie: 0 });
      b[h.type === 'tv' ? 'tv' : 'movie']++;
    }
    db.analytics = a;
    await savePayload();
  ...
  v.appendChild(el('div', 'section-head', 'App'));
  v.appendChild(el('div', 'muted',
    `Build <b style="color:var(--gold-lite)">${BUILD}</b> · ${history.length.toLocaleString()} history · ${db.shows.length} shows`));
  const upd = el('button', 'btn gold', 'Force update');
  upd.style.cssText = 'width:100%;margin-top:8px';
  upd.onclick = async () => {
    // Unregister the service worker and drop every cache, then hard reload.
    // This is the reliable way to prove whether new code has landed.
    try {
      ...
      Object.keys(localStorage).filter(k => k.startsWith('tmdb:') || k.startsWith('fb:') || k === 'mytv.feeds')
        .forEach(k => localStorage.removeItem(k));
    } catch { /* best effort */ }
  ...
  v.appendChild(upd);
  v.appendChild(el('div', 'muted',
    'Clears caches and reloads. Your keys and data are untouched.'));

  v.appendChild(el('div', 'section-head', 'Device'));
...
async function openActor(actorId, actorName) {
  ...
  roles.appendChild(el('div', 'section-head', 'Known for'));
  roles.appendChild(gridOf(top.map(c => ({
    title: c.name || c.title, poster: c.poster_path,
    badge: c.media_type === 'tv' ? 'Series' : 'Film',
    onOpen: () => c.media_type === 'tv' ? openShow(c.id, c.name) : openMovie(c.id, c.title) }))));
}
...
async function openShow(showId, showName) {
  ...
  const drawSeason = () => {
    ...
    for (const ep of seasonEps(b, n)) {
      ...
      row.innerHTML = `<div class="ep-thumb">
          ...
          ${on ? '<button class="ep-notes">📝 Notes</button>' : ''}</div>
        <div class="ep-body">
      ...
      if (chk && !chk.disabled) chk.onclick = async () => {
        ...
        drawSeason(); toast(on ? 'Unmarked' : `${code} logged`);
      };
  ...
  body.querySelector('.act-season').onclick = async (e) => {
    ...
    await markMany(showId, codes, true);
    e.target.disabled = false;
    drawSeason(); toast(`Season ${n} logged`);
  };
...
async function openFootball(showId, showName) {
  ...
  const draw = () => {
    ...
    days[md].forEach((g, i) => {
      ...
      if (btn) btn.onclick = async () => {
        btn.disabled = true;
        await setEpisode(showId, cs, !on, { name: showName });
        if (on) watched.delete(cs); else watched.add(cs);
        draw(); toast(on ? 'Unmarked' : 'Logged');
      };
  ...
  if (all) all.onclick = async () => {
    ...
    all.disabled = false; draw(); toast(`Matchday ${md} logged`); render();
  };
  ...
  if (none) none.onclick = async () => {
    ...
    await markMany(showId, codes, false);
