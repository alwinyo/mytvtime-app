...
const BUILD = '2026-08-24-i';      // shown in Me -> Data; bump on every upload
...
function sheetHeader(title, backdrop, poster, chips) {
  // A real 16:9 backdrop reads as a cover image; falling back to the poster
  // means cropping a 2:3 image into a wide box, which is why it looked wrong.
  // Poster fallbacks get heavier blur so they read as texture, not a bad crop.
  const hasBackdrop = !!backdrop;
  const bg = hasBackdrop ? imgUrl(backdrop, 'w780')
           : (isAbs(poster) ? '' : imgUrl(poster, 'w780'));
  const cover = bg
    ? `<div class="sheet-hero${hasBackdrop ? '' : ' from-poster'}">
         <img src="${bg}" alt="">
       </div>`
...
async function openEpisode(showId, showName, code) {
  ...
  body.innerHTML = sheetHeader(ep.name || showName, show.backdrop_path || ep.still_path, show.poster_path,
