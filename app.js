...
async function computeNext() {
  ...
  for (const s of db.shows) {
    if (s.dropped || isFootballShow(s)) continue;
  ...
  rows.push(...await computeFootballRows('next'));
...
async function computeSoon() {
  ...
  rows.push(...await computeFootballRows('soon'));
...
async function renderNext(v) {
  ...
  h.innerHTML = `<img src="${imgUrl(hero.backdrop || hero.poster, 'w780')}" alt="">
    <span class="hero-inner"><span class="hero-eyebrow">Up next</span>
      ...
      <span class="hero-ep">${esc(hero.code)} · ${esc(hero.epName)}</span></span>`;
...
function renderLibrary(v) {
  ...
  if (libKind === 'tv') {
    items = db.shows.filter(s => {
      ...
      onOpen: () => isFootballShow(s) ? openFootball(s.id, s.name) : openShow(s.id, s.name) }));
  } else {
...
async function renderHealth(v) {
  ...
  for (const s of db.shows) {
    ...
    if (rem > 0 && rem <= 3 && !isFootballShow(s)) almost.push({ name: s.name, rem });
  }
  ...
  if (almost.length) {
    ...
    almost.slice(0, 6).forEach(a => v.appendChild(el('div', 'line', `<b>${esc(a.name)}</b> — ${a.rem} left`)));
  }
}
...
async function openMovie(id, name) {
  ...
  body.innerHTML = sheetHeader(d.title || name, d.backdrop_path, d.poster_path, chips) +
    ...
    `<div class="sheet-actions">
       <button class="btn ${on ? '' : 'gold'} act-toggle">${on ? 'Unmark' : 'Mark watched'}</button>
       <button class="btn act-lib">${inLib ? 'Remove' : 'Add to library'}</button></div></div>`;
  const inner = body.querySelector('.sheet-body');
  body.querySelector('.act-toggle').onclick = async (e) => {
    e.target.disabled = true;
  ...
  body.querySelector('.act-lib').onclick = async () => {
    if (inLib) db.movies = db.movies.filter(x => String(x.id) !== String(id));
