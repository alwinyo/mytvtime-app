...
const BUILD = '2026-08-24-a';      // shown in Me -> Data; bump on every upload
...
function renderData(v) {
  ...
  const over = db.shows.filter(x => !x.dropped && !isFootballShow(x)
    && x.total_episodes > 0 && x.watched_episodes.length > x.total_episodes);
  repair.appendChild(dataRow('Refresh episode totals',
    over.length ? `${over.length} show${over.length > 1 ? 's' : ''} show more watched than exist — TMDB has since added episodes.`
                : 'Re-reads episode counts from TMDB for every show.',
    'Refresh', async (b) => {
