...
const BUILD = '2026-08-23-j';      // shown in Me -> Data; bump on every upload
...
function renderData(v) {
  ...
  app.innerHTML += `<div class="kv"><span>History depth</span><b>${history.length}</b></div>`;
  const blog = el('div', 'muted',
    backLog.length ? backLog.join('<br>') : 'No back events yet — open a show, then swipe back.');
  app.appendChild(el('div', 'section-head', 'Back gesture log'));
...
*/
const backLog = [];                 // shown in Me -> Data for diagnosis

function noteBack(what) {
  backLog.unshift(`${stamp().slice(11)} ${what} (len ${history.length})`);
  backLog.length = Math.min(backLog.length, 6);
}

function pushHistory(state) {
  try {
    history.pushState({ mytv: true, ...state }, '');
    noteBack(`push ${state.kind}`);
  } catch (e) {
    noteBack(`push failed: ${e.message}`);
  }
}

window.addEventListener('popstate', (e) => {
  noteBack(`pop ${e.state?.kind || 'none'}`);
...
function boot() {
  try { history.replaceState({ mytv: true, kind: 'root', view: 'next' }, ''); } catch { /* ignore */ }
